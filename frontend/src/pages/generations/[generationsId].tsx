...
const EditGenerations = () => {
  ...
  const { generationsId } = router.query

  useEffect(() => {
    dispatch(fetch({ id: generationsId }))
  }, [generationsId])
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: generationsId, data }))
...
EditGenerations.getLayout = function getLayout(page: ReactElement) {
  return (
      <LayoutAuthenticated>
          {page}
      </LayoutAuthenticated>
